        ...
        <div class="inner">
            <div class="pad">
                ...
                <div class="col-md-10 mr-auto ml-auto">
                    ...
                    @endif
                    <form id="JobSheetCompanyRegisterSubimageForm" action="{{route('update.jobsheet.subimage1', [$jobitem])}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('POST')
                        ...
                        <input type="hidden" name="data[File][currentPath]" value="@if(config('app.env') == 'production'){{ config('app.s3_url') . '/img/uploads/JobSheet/' . $jobitem->job_img_2 . '?' . date('YmdHis') }}@else{{'https://job-cinema-dev.s3-ap-northeast-1.amazonaws.com/img/uploads/JobSheet/' . $jobitem->job_img_2 . '?' . date('YmdHis') }}@endif" id="FileCurrentPath">
                        @else
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                @if($jobitem->job_img_2)
                                <p class="pre-main-image"><img src="@if(config('app.env') == 'production'){{ config('app.s3_url') . '/img/uploads/JobSheet/' . $jobitem->job_img_2 . '?' . date('YmdHis') }}@else{{'https://job-cinema-dev.s3-ap-northeast-1.amazonaws.com/img/uploads/JobSheet/' . $jobitem->job_img_2 . '?' . date('YmdHis') }}@endif" alt="サブ写真を登録してください"></p>
                                @endif
                                <ul class="list-unstyled">
...
<script>
    $(function() {
        ...
        if (imagePath != '') {
            window.opener.$('#photo2').attr('src', imagePath);
            if (imagePath == '/img/common/no-image.gif') {
                window.opener.$('#FileIsExist2').val(0);
            } else {
                window.opener.$('#FileIsExist2').val(1);
            }
        }

        $('#deleteImage').click(function() {
            if (window.confirm('登録されているサブ写真を削除します。よろしいですか？')) {
                window.location.href = '/company/jobs/create/delete_image/' + job.id + '?flag=2';
                return false;
            }
        });
    });
