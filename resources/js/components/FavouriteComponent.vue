  ...
  methods: {
        ...
        .then(response => {
          if (response.data.fav_save_status == 1) {
            ...
            document.getElementById(
              "hamburgerLogoutClipJobCount"
            ).innerHTML = this.count[2];
            alert("お仕事情報を保存しました。");
          } else {
    ...
    unsave() {
      if (confirm("選択した求人情報を削除しますか？")) {
        this.startProcessing();
        axios
          .post("/keeplist/unsave/" + this.jobid)
          .then(response => {
            if (response.data.fav_del_status == 1) {
              ...
              if (this.count[0] > 0 && this.count[1] > 0) {
                ...
                document.getElementById(
                  "saveCount-pc"
                ).innerHTML = this.count[0];
                document.getElementById(
                  "saveCount-sp"
                ).innerHTML = this.count[1];
                document.getElementById(
                  "hamburgerLogoutClipJobCount"
                ).innerHTML = this.count[1];
              }
