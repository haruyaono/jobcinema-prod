<template>
 <div class="search-history-section">
    <h2 class="txt-h2 top-heading-h2"><i class="fas fa-history font-yellow mr-2"></i>検索履歴</h2>
      ...
      <ul class="search-history-list">
        <li v-for="(searchItem, index) in searchItems" :key="searchItem.id">
          <dl class="p-mypage-search-condition-text">
            <dt>履歴{{index + 1}}</dt>
            <dd>
              <a v-bind:href="'/jobs/search/all?' + params[index] " class="txt-blue-link">{{ nameList['no'+index] }}</a>
               
              {{searchItem.count}}件
            </dd>
          </dl>
