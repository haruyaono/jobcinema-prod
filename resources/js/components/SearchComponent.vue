...
<section class="search-section">
  <div class="inner">
      <form id="composite-form" method="get" action="/jobs/search/all" role="form" class="cf">
        <div v-for="(category, index) in categories" :key="category.id" v-bind:class="'composite composite-' + index">
          <div class="composite-left">
            <p>{{category.name}}<span class="ib-only-pc">で探す</span></p>
          </div>
          <div class="composite-right">
            <div v-bind:class="'select-wrap select-wrap-' + category.slug">
              <select v-if="category.slug == 'salary'" v-bind:id="'search-' + category.slug " class="selectbox" v-bind:name="category.slug" v-model="params[category.slug]" v-on:change="fetchSalaries">
                <option value="">選択してください</option>
                  <option v-for="cat in category.children" :key="cat.id" v-bind:value="cat.id" v-bind:data-val="cat.name" ref="category">{{ cat.name }}</option>
              </select>
              <select v-else v-bind:id="'search-' + category.slug " class="selectbox" v-bind:name="category.slug" v-model="params[category.slug]">
                <option value="">選択してください</option>
                  <option v-for="cat in category.children" :key="cat.id" v-bind:value="cat.id" v-bind:data-val="cat.name" ref="category">{{ cat.name }}</option>
              </select>
            </div>
            <div v-if="category.slug == 'salary'" class="option-lst jsc-option-lst jsc-option-required-wrapper select-wrap"> 
                <select v-bind:name="salaries['slug']" id="salary-child" class="selectbox" v-model="params.salary_child[salaries['slug']]">
                  <option value="">指定なし</option>
                    <option  v-for="cCat in salaries['children']" :key="cCat.id" v-bind:value="cCat.id">{{ cCat.name }}</option>
                </select>
            </div>
          </div>
        </div>
        <div class="composite mb-3 composite-5">
          ...
          <div class="composite-right">
            <input id="search-text" type="text" name="keyword" style="height:35px; padding: 4px 6px; width: 100%;" placeholder="例）コンビニ" value="" v-model="params.keyword">  
          </div>
        </div>

        <p class="job-count">検索結果 <span id="job-count">{{params.count}}</span>件</p>

        <button type="submit" id="filter-search" @click="search()"><i class="fas fa-search" ></i>絞り込み検索</button>
      </form>
  </div> <!-- inner -->
...
export default {
    ...
    methods: {
        fetchSalaries: function() {
            ...
            Object.keys(self.params.salary_child).forEach(key => self.params.salary_child[key] = '');
